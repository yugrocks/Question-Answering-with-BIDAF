...
def model():
    ...
    S = Lambda(get_S_matrix_alternate,name="S_calculation")([contextual_embed_query, contextual_embed_context]) # shape J x T
    ...
    p1 = Lambda(lambda x: K.softmax(x,axis=1))(p1)
    p2 = Lambda(lambda x: K.softmax(x,axis=1))(p2)
...
#d = model.predict(x=[np.random.randn(32,query_len,embed_len),np.random.randn(32,context_len,embed_len)] )
